#!/usr/bin/env bash
# СДВИГ R21 — match-3 v9: полный DOM/CSS движок (GPU), фишки-div'ы не исчезают, бустеры видны
set -e

echo ""; echo "══ match3.js → v9 (DOM/CSS, без Canvas) ═════════════"
M3="src/main/resources/static/games/match3.js"
cp "$M3" "${M3}.v8.bak" 2>/dev/null || true
cat > "$M3" << 'M3_EOF'
/* ═══════════════════════════════════════════════════════════
...
M3_EOF
echo "✓ match3.js → v9 (старый → match3.js.v8.bak)"

echo ""
echo "═══════════════════════════════════════════════════════"
echo "✅  R21 готов — match-3 переписан на DOM/CSS"
echo "   git add -A && git commit -m 'R21: match3 v9 DOM/CSS engine - GPU, no vanish, real shine' && git push"
echo "═══════════════════════════════════════════════════════"
