#!/usr/bin/env bash
# СДВИГ R20 — match-3 v8: фикс пропажи фишек, перелив, без лагов, на весь экран
set -e

echo ""; echo "══ 1/2  match3.js → v8 ══════════════════════════════"
M3="src/main/resources/static/games/match3.js"
cp "$M3" "${M3}.v7.bak" 2>/dev/null || true
cat > "$M3" << 'M3_EOF'
/* ═══════════════════════════════════════════════════════════
...
M3_EOF
echo "✓ match3.js → v8 (старый → match3.js.v7.bak)"

echo ""; echo "══ 2/2  CSS — модалка match-3 на весь экран ════════"
python3 - << 'PYEOF'
...
if "/* R20 */" in txt:
    ...
    css = '''
/* ════ R20 — match-3 на весь экран ════ */
.hint-modal{ height:100dvh !important; height:100vh; border-radius:0 !important; }
.hint-modal .hm-vp{ flex:1 1 auto; min-height:0; }
/* футер компактнее, чтобы поле было больше */
.hint-modal .hm-footer{ padding:8px 18px calc(8px + var(--safeb)) !important; }
.hint-modal .hm-header{ padding:12px 18px !important; }
'''
    txt += "\n/* R20 */\n" + css
    with open(path, "w", encoding="utf-8") as f: f.write(txt)
    print("  + модалка на весь экран (100dvh)")
PYEOF

echo ""
echo "═══════════════════════════════════════════════════════"
echo "✅  R20 готов — match-3 v8 (фиксы)"
echo "   git add -A && git commit -m 'R20: match3 v8 fixes - no vanish, shine, fullscreen, no lag' && git push"
echo "═══════════════════════════════════════════════════════"
