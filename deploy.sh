#!/usr/bin/env bash
# СДВИГ R22 — match-3 v10: фикс невидимых фишек, производительность, усилители Jewels-Planet
set -e
echo ""; echo "══ match3.js → v10 ══════════════════════════════════"
M3="src/main/resources/static/games/match3.js"
cp "$M3" "${M3}.v9.bak" 2>/dev/null || true
cat > "$M3" << 'M3_EOF'
/* ═══════════════════════════════════════════════════════════
...
M3_EOF
echo "✓ match3.js → v10 (старый → match3.js.v9.bak)"
echo ""
echo "═══════════════════════════════════════════════════════"
echo "✅  R22 готов — match-3 v10"
echo "   git add -A && git commit -m 'R22: match3 v10 - fix invisible gems, perf, Jewels-style boosters' && git push"
echo "═══════════════════════════════════════════════════════"
