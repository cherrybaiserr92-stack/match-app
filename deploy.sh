#!/bin/bash
# ═══════════════════════════════════════════════════════════════
#  СДВИГ · deploy.sh — 2D карта-город (рисованные кварталы)
# ═══════════════════════════════════════════════════════════════
set -e
S="src/main/resources/static"
echo ""
echo "🏙  СДВИГ — 2D карта города…"
echo ""
echo "  ✦ $S/style.css"
...
.map-scroll{ position:absolute; inset:0; overflow-y:auto; -webkit-overflow-scrolling:touch;
  background:linear-gradient(180deg, #0a0e16 0%, #0d1018 40%, #0a0c12 100%);
}
...
/* табличка района */
.map-chapter{ position:absolute; transform:translate(-50%,0) rotate(-1deg); z-index:3; text-align:center; white-space:nowrap;
  background:linear-gradient(180deg,#fdf6e3,#e8dcc0); color:#2a2012;
  padding:7px 18px; border-radius:8px; box-shadow:0 6px 18px rgba(0,0,0,.5), inset 0 1px 0 rgba(255,255,255,.6); }
...
echo ""
echo "✅  Готово!"
echo "  git add -A && git commit -m \"feat: 2D illustrated city map with drawn districts\" && git push"
