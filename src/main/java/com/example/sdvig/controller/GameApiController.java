...
public class GameApiController {
    ...
    public ResponseEntity<?> authWebApp(@RequestBody Map<String, Object> payload) {
        try {
            ...
            if (!authService.validateWebAppInitData(initData)) {
                return ResponseEntity.status(401).body("Invalid WebApp signature. Проверьте токен.");
            }
    ...
    // ИСПРАВЛЕНО: Принимаем Object, чтобы не падать на числах
    ...
    public ResponseEntity<?> authWidget(@RequestBody Map<String, Object> payload) {
        try {
            if (!authService.validateWidgetAuth(payload)) {
                return ResponseEntity.status(401).body("Invalid widget signature. Проверьте токен.");
            }
            
            Object idObj = payload.get("id");
            if (idObj == null) return ResponseEntity.status(400).body("Missing ID");
            
            return processUser(String.valueOf(idObj),
    ...
    public ResponseEntity<?> claimDailyBonus(@RequestParam String providerId) {
        ...
        if (today.equals(last)) return ResponseEntity.badRequest().body("Бонус уже получен сегодня.");
    ...
    public ResponseEntity<?> advanceLevel(@RequestParam String providerId, @RequestParam String gameType) {
        PlayerProfile p = profileRepo.findByProviderId(providerId).orElse(null);
        if (p == null) return ResponseEntity.badRequest().body("Profile not found");
