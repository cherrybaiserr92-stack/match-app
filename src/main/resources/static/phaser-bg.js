(function(){
  ...
  function makeTex(scene){
    ...
    for(let s=0;s<4;s++){ const y=H*0.2+s*H*0.18;
      ...
      for(let b=0;b<7;b++){ g.fillStyle(0x1a2336,0.45).fillRect(W*0.05+b*W*0.055,y-28-(b%3)*6,W*0.04,28+(b%3)*6);} }
  ...
  function create(){
    ...
    scene._rain=[]; for(let i=0;i<28;i++) scene._rain.push({x:Math.random()*W, y:Math.random()*H, l:8+Math.random()*8, s:5+Math.random()*5});
    drawRain(W,H);
  ...
  function drawRain(W,H){
    ...
    scene._rain.forEach(r=>{ rain.beginPath(); rain.moveTo(r.x,r.y); rain.lineTo(r.x-2,r.y+r.l); rain.strokePath(); });
  }
  ...
  window.BgFx={
    init:boot, pause(){ paused=true; if(game) game.loop.sleep(); }, resume(){ paused=false; if(game) game.loop.wake(); }
  };
